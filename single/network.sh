...
function checkPrereqs() {
  ...
  if [ "$LOCAL_VERSION" != "$DOCKER_IMAGE_VERSION" ]; then
    warnln "Local fabric peer binaries and docker images are out of  sync. This may cause problems."
  fi

  ORDER_LOCAL_VERSION=$(orderer version | sed -ne 's/ Version: //p')
  ORDER_DOCKER_IMAGE_VERSION=$(docker run --rm hyperledger/fabric-orderer:$IMAGETAG orderer version | sed -ne 's/ Version: //p' | head -1)

  infoln "ORDER_LOCAL_VERSION=$ORDER_LOCAL_VERSION"
  infoln "ORDER_DOCKER_IMAGE_VERSION=$ORDER_DOCKER_IMAGE_VERSION"

  if [ "$ORDER_LOCAL_VERSION" != "$ORDER_DOCKER_IMAGE_VERSION" ]; then
    warnln "Local fabric orderer binaries and docker images are out of  sync. This may cause problems."
  fi
...
function networkDown() {
  # stop ca, org1 and org2 containers
...
function networkClean() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run --rm -v $(pwd):/data hyperledger/fabric-peer sh -c 'cd /data && rm -rf system-genesis-block/*.block organizations/peerOrganizations organizations/ordererOrganizations'
    ...
    docker run --rm -v $(pwd):/data hyperledger/fabric-peer sh -c 'cd /data && rm -rf channel-artifacts log.txt *.tar.gz'
...
CHANNEL_NAME="hxyz"
# chaincode name defaults to "archive"
CC_NAME="archive"
...
COMPOSE_FILE_BASE=docker/docker-compose-produce-net.yaml
