...
function checkPrereqs() {
  ...
  if [ ! -f "organizations/fabric-ca/org2/tls-cert.pem" ]; then
    ...
    warnln "copy 'organizations/fabric-ca/org2/tls-cert.pem' file from fabric ca server 'organizations/fabric-ca/org2/tls-cert.pem'."
    exit 1
  fi
}
...
function networkUp() {
  ...
  docker run --env CORE_VM_ENDPOINT=unix:///host/var/run/docker.sock --env CORE_VM_DOCKER_HOSTCONFIG_NETWORKMODE=docker_produce --env FABRIC_LOGGING_SPEC=INFO --env CORE_PEER_TLS_ENABLED=true --env CORE_PEER_PROFILE_ENABLED=true --env CORE_PEER_TLS_CERT_FILE=/etc/hyperledger/fabric/tls/server.crt --env CORE_PEER_TLS_KEY_FILE=/etc/hyperledger/fabric/tls/server.key --env CORE_PEER_TLS_ROOTCERT_FILE=/etc/hyperledger/fabric/tls/ca.crt --env CORE_PEER_ID=peer0.org2.example.com --env CORE_PEER_ADDRESS=peer0.org2.example.com:9051 --env CORE_PEER_LISTENADDRESS=0.0.0.0:9051 --env CORE_PEER_CHAINCODEADDRESS=peer0.org2.example.com:9052 --env CORE_PEER_CHAINCODELISTENADDRESS=0.0.0.0:9052 --env CORE_PEER_GOSSIP_BOOTSTRAP=peer0.org2.example.com:9051 --env CORE_PEER_GOSSIP_EXTERNALENDPOINT=peer0.org2.example.com:9051 --env CORE_PEER_LOCALMSPID=Org2MSP --network docker_produce -p 9051:9051 -v /var/run/:/host/var/run/ -v $(pwd)/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/msp:/etc/hyperledger/fabric/msp -v $(pwd)/organizations/peerOrganizations/org2.example.com/peers/peer0.org2.example.com/tls:/etc/hyperledger/fabric/tls -v $(pwd)/peer0.org2.example.com:/var/hyperledger/production -w /opt/gopath/src/github.com/hyperledger/fabric/peer --name=peer0.org2.example.com hyperledger/fabric-peer:latest peer node start
...
COMPOSE_FILE_BASE=docker/docker-compose-org2-produce-net.yaml
