...
function createCAs() {
  ...
  docker run --env FABRIC_CA_HOME=/etc/hyperledger/fabric-ca-server --env FABRIC_CA_SERVER_CA_NAME=ca-org1 --env FABRIC_CA_SERVER_TLS_ENABLED=true --env FABRIC_CA_SERVER_PORT=7054 --network docker_produce -p 7054:7054 -v $(pwd)/organizations/fabric-ca/org1/:/etc/hyperledger/fabric-ca-server --name=ca_org1 hyperledger/fabric-ca:latest sh -c 'fabric-ca-server start -b admin:adminpw -d'
